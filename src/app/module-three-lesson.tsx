...
import {
  ...
  getModuleThreeLesson,
  getNextModuleThreeLesson,
  isModuleThreeLessonUnlocked,
  MODULE_THREE_LESSON_COUNT,
  MODULE_THREE_LESSONS,
  type ModuleThreeLessonId,
} from "../lib/curriculum";
import { MODULE_THREE_CONTENT } from "../lib/module-three-content";

export default function ModuleThreeLessonScreen() {
  ...
  const requestedLesson = getModuleThreeLesson(params.lessonId);
  const lesson =
    requestedLesson &&
    isModuleThreeLessonUnlocked(requestedLesson.id, progress.completedLessonIds)
      ? requestedLesson
      : getCurrentModuleThreeLesson(progress.completedLessonIds);
  const lessonId = lesson.id as ModuleThreeLessonId;
  const lessonIndex = MODULE_THREE_LESSONS.findIndex((item) => item.id === lessonId);
  const content = MODULE_THREE_CONTENT[lessonId];
  ...
  const nextLesson = getNextModuleThreeLesson(lessonId);
  const isFinalLesson = lessonIndex === MODULE_THREE_LESSON_COUNT - 1;
  ...
  return (
    <>
        ...
        <View style={styles.appFrame}>
          <View style={styles.header}>
            ...
            <Text style={styles.stepLabel}>
              {lessonIndex + 1} of {MODULE_THREE_LESSON_COUNT}
            </Text>
          </View>
          ...
          <View style={styles.progressTrack}>
            <View
              style={[
                styles.progressFill,
                { width: `${((lessonIndex + 1) / MODULE_THREE_LESSON_COUNT) * 100}%` },
              ]}
            />
          </View>
          ...
          >
            <View style={styles.intro}>
              <Text style={styles.eyebrow}>Color & light</Text>
            ...
            <View style={styles.workspace}>
              ...
              <View style={styles.previewCard}>
                ...
                <View style={styles.previewFooter}>
                  <Text style={styles.previewLabel}>Color field</Text>
      ...
      <LessonCompletionSheet
        completionMessage={
          isFinalLesson
            ? "Module 03 is complete. Procedural Textures is now unlocked."
            : undefined
        }
        lessonTitle={lesson.title}
        nextActionLabel={nextLesson ? "Continue to next lesson" : "Return to course"}
...
const styles = StyleSheet.create({
  ...
  previewFooter: { minHeight: 44, paddingHorizontal: Spacing.md, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  ...
  codeLine: { minHeight: 25, paddingHorizontal: Spacing.md, flexDirection: "row", alignItems: "flex-start" },
  lineNumber: { width: 24, color: Colors.textSubtle, fontFamily: "monospace", fontSize: 11, lineHeight: 18 },
  codeText: { flex: 1, color: Colors.textMuted, fontFamily: "monospace", fontSize: 11, lineHeight: 18 },
